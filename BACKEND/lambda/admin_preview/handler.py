"""
admin_preview Lambda - Generate CloudFront signed URL for admin preview books

Triggered by GET /books/{bookId}/preview-url
Returns a signed CloudFront URL that expires in 1 hour.

Flow:
1. User authenticated via JWT (admin)
2. Check if book exists and is pending
3. Generate CloudFront signed URL
4. Return URL to frontend
5. Frontend uses URL to read file from CloudFront and use iframe to read
...
def _get_book_file_path(book_id: str, table_name: str) -> Optional[str]:
    ...
    Returns:
        File path (e.g., staging/90b5a1e4-3733-4bf9-a4e4-35b2f0607ccb/lolita.pdf) or None if not found
    """
    try:
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda handler for GET /books/{bookId}/preview-url
